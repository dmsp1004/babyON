...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      ...
      // 토큰 및 사용자 정보 저장 (안전한 타입 변환)
      _token = responseData['token']?.toString();

      // userId는 숫자 타입일 수 있으므로 안전하게 변환
      ...
      if (userIdValue is int) {
        ...
        _userId = int.tryParse(userIdValue.toString());
      }

      _email = email;
      _userType = responseData['role']?.toString(); // 백엔드에서 제공하는 사용자 유형(role) 저장
  ...
  Future<bool> handleSocialLoginCallback(String code, String provider) async {
    ...
    try {
      ...
      print('소셜 로그인 응답 데이터: $loginResult');

      // 안전한 타입 변환
      _token = loginResult['token']?.toString();

      final userIdValue = loginResult['userId'];
      if (userIdValue is int) {
        _userId = userIdValue;
      } else if (userIdValue is String) {
        _userId = int.tryParse(userIdValue);
      ...
      _email = loginResult['email']?.toString();
      _userType = loginResult['role']?.toString();

      // 유효성 검증
      if (_token == null || _userId == null || _email == null || _userType == null) {
      ...
      print('소셜 로그인 처리 예외: $_errorMessage');
      _isLoading = false;
      _isAuthenticating = false;
      notifyListeners();
      return false;
  ...
  Future<bool> register(
    ...
    try {
      ...
      print('회원가입 응답 데이터: $responseData');
